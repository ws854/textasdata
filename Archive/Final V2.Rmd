...
hist(BNC_FRE, breaks=500, col="red",xlim = c(0,100))
...
```{r}
# in top 100 
print(setdiff(names(top_american)[1:100],names(top_british)[1:100])) 
print(setdiff(names(top_british)[1:100],names(top_american)[1:100])) 
...
dale_full_df[with(dale_full_df, order(-diff)), ]
```
...
BNC_dale_weight_remove <- BNC_dale_freq_remove/sum(BNC_dale_freq_remove)
OANC_dale_weight_remove <- OANC_dale_freq_remove/sum(OANC_dale_freq_remove)
...
mean(dale_full_df_rm$BNC_weight) - mean(dale_full_df_rm$OANC_weight)
var(dale_full_df_rm$BNC_weight) - var(dale_full_df_rm$OANC_weight)


dale_full_df_rm[with(dale_full_df_rm, order(-diff)), ]
