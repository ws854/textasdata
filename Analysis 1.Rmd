...
plot(xfit, yfit, xlim=c(0,100))
par(new=TRUE)
plot(xfit1, yfit1, xlim=c(0,100))
...
full_df[with(full_df, order(-difference)), ]

a <- kwic(OANC_corpus,"dole",10)
```
...
dale_full_df[with(dale_full_df, order(-diff)), ],"dale_incl.csv")
```
...
write.csv(dale_full_df_rm[with(dale_full_df_rm, order(-diff)), ],"dale_rm.csv")
...
#boostrap 
```{r}
downsize_boc <- corpus_sample(BNC_sub_corpus, 500, replace = FALSE )
downsize_dfm <- dfm(downsize_boc,stem = F, remove_punct = TRUE, tolower = T)
x<-dfm_trim(BNC_dfm,min_count = 30,min_docfreq =20)

for(i in 1:100){
  british_sample <- downsize
